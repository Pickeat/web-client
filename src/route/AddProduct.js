...
const labelList = ['fresh', 'gluten_free', 'halal', 'kosher', 'vegan', 'veggie'];

function getStyles(labelList, labels) {
  return {
    fontFamily: labelList.indexOf(labels) === -1 ? 'Colfax-regular' : 'Colfax-medium',
  };
}
      ...
      navigator.geolocation.getCurrentPosition(
        (location) => {
          ...
          addProductApi(image, title, userLocation, description, date, labels).then((e) => {
            if (e != null) history.push('/product-list');
          });
        },
        () => {
        ...
        <form className={classes.form}>
          ...
          <div className={classes.imageInputSection}>
            <PickeatTextField
              ...
              value={imageName}
            />
            <Button style={{ width: '15%', height: '55px' }} variant="contained" component="label">
              {' '}
              <AddIcon />
