...
export default function ProductList(props) {
  ...
  const getProductList = (params) => {
    ...
    if (params.search === undefined) search = searchValue;
    getProductListApi(
      8,
      page - 1,
      search,
      params.km || sliderValue,
      location,
      params.rate || minRate,
      params.date || maxDate,
    ).then((response) => {
  ...
  const handleViewProductMode = (event, newMode) => {
    if (newMode) SetViewProductMode(newMode);
  };
  ...
  return (
    <div className={classes.main}>
        ...
        <Paper elevation={5} className={classes.paramsSection}>
          ...
          <div className={classes.filterContainer}>
            <KmSlider
              ...
              handleInputChange={handleKmChange}
      ...
      <div className={classes.rightSection}>
        {BuildToggleButton()}
        <div className={classes.productListSectionContainer}>{BuildProductList()}</div>
      </div>
      <div style={{ position: 'absolute', right: '40px', bottom: '40px' }}>
        <Fab
          ...
          style={{ backgroundColor: 'white', color: '#40ee49' }}
          aria-label="add"
        >
          <AddIcon />
        </Fab>
